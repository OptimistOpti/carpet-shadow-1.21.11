...
// FIXME [PORT 1.21.11] see PORTING_NOTES.md #1.
//  ItemStack.fromNbt(NbtCompound)/writeNbt(NbtCompound) no longer exist in this form since 1.20.5
//  (Data Components rewrite). Find the current (de)serialization entry points in the decompiled
//  1.21.11 sources (search for ItemStack's static decode/encode-style methods that still take a
//  plain NbtCompound for the "legacy"/single-player-save path) and re-target these injectors there.
    ...
    private static ItemStack post_fromNbt(ItemStack stack, NbtCompound nbt) {
        if (nbt.contains("shadow")) {
            if(CarpetShadowSettings.shadowItemMode.shouldResetCount()){
                ...
                String shadow_id = nbt.getString("shadow").orElse(null);
